...
export function TreeNavigation({
	...
	const renderItem = (item: FolderType) => {
		...
		return (
			...
			>
				...
				{/* Icon của mục */}
				<i
					className={`${item.icon || (item.isFile ? "bi-file-earmark-text" : "bi-folder")} mr-1 transition-transform duration-200 group-hover:scale-110`}
				/>
	...
	const renderChildren = () => {
		...
		return Object.values(folder.children)
			.filter((child) => (showDeleted ? child.deleted : !child.deleted)) // Lọc theo `showDeleted`
			.map((child) => {
